...
w_power:connect_signal("button::press", function(_, _, _, button)
    if button == 1 then
        awful.spawn.with_shell(os.getenv("HOME") .. "/.scripts/awesome_shutdown_ui")
    end
end)

local w_sticky_terminal = sticky_terminal({terminal_cmd = "kitty --class umbe_sticky_terminal tmux new -A -s sticky"})
